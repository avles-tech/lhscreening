<!-- <?php $patient_reports = $this->patient_reports_model->get_patient_reports($patient_id); 
 ...
 <?php $patient_reports = $this->patient_reports_model->get_patient_reports($patient_id); 
	if (!empty($patient_reports[$report])) :
        echo "<a class='btn btn-primary'  role='button' href='".base_url()."index.php/upload/download/".$patient_reports[$report]."'>".$patient_reports[$report]."</a>";
        echo "<a class='btn btn-warning' onclick='ChangeFileNameEx(this)' data-filename='".$patient_reports[$report]."' data-report='".$report."' role='button' href='#' > rename </a>";
        echo "<a class='btn btn-danger'  role='button' href='#' onclick='delReportEx(this)' data-report='".$report."'> delete </a>";
 endif ?>
